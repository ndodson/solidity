...
Stack OptimizedEVMCodeTransform::tryCreateStackLayout(Stack _currentStack, Stack const& _targetStack, vector<VariableSlot> const& _currentFunctionReturnVariables)
{
	Stack unreachable;
	::createStackLayout(_currentStack, _targetStack, [&](unsigned _i) {
		if (_i > 16)
		{
			if (!util::findOffset(unreachable, _currentStack.at(_currentStack.size() - _i - 1)))
				unreachable.emplace_back(_currentStack.at(_currentStack.size() - _i - 1));
		}
	}, [&](StackSlot const& _slot) {
		if (canBeFreelyGenerated(_slot))
			return;
		if (auto depth = util::findOffset(_currentStack | ranges::views::reverse, _slot); depth && *depth >= 16)
		{
			if (depth)
			{
				if (*depth >= 16)
			...
			{
				VariableSlot const* varSlot = get_if<VariableSlot>(&_slot);
				yulAssert(varSlot && util::findOffset(_currentFunctionReturnVariables, *varSlot), "");
			}
		}
	}, [&]() {});
	return unreachable;
